...
char* WideCharToAnsi(const WCHAR* ws)
{
  size_t len = WideCharToMultiByte(CP_ACP, 0, ws, -1, 0, 0, 0, 0);
  char* rval = NULL;

  if (len)
  {
    rval = (char*) GlobalAlloc(LPTR, (len + 1));
    WideCharToMultiByte(CP_ACP, 0, ws, -1, rval, len+1, 0, 0);
  }

  return rval;
}

static DWORD dwRead;        // Offset from dwCookie to read from.
static BOOL bCookieNuked;   // Mark dwCookie as having been deleted.
static BOOL bManageCookie;  // Set to true if we need to delete dwCookie.

// Streams the ASCII version of the license text.  This would also include
// RTF files.
DWORD CALLBACK StreamLicenseA(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
  if (!bCookieNuked)
  {
    ...
    *pcb=lstrlenA(pbBuff);
    ...
    if (cb > 0 && *pcb == 0 && bManageCookie)
    {
      // Nothing was read which means we're done.
      GlobalFree((HGLOBAL) dwCookie);
      bCookieNuked = TRUE;
    }
  }
  else /* Nuked! */
...
// Streams the Unicode version of the license text.
DWORD CALLBACK StreamLicenseW(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
  size_t len;
  size_t count = cb/sizeof(WCHAR);
  
  // We don't want zero count reads.
  if (count == 0) count = 1;

  lstrcpynW((WCHAR*)pbBuff, ((WCHAR*)dwCookie) + dwRead, count);
  len  = lstrlenW((WCHAR*)pbBuff);
