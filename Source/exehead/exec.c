...
#define parm5 (lent.offsets[5])
  ...
  switch (which)
  {
    ...
    case EW_GETFONTVERSION:
      {
        TCHAR *version=var0;
        TCHAR* buf1=GetStringFromParm(0x11);
        if (!GetTTFVersionString(buf1, version, NSIS_MAX_STRLEN))
        {
          exec_error++;
        }
        log_printf3(_T("GetTTFVersionString(%s) returned %s"),buf1,version);
      }
      break;
#endif//NSIS_SUPPORT_GETFONTVERSION
#ifdef NSIS_SUPPORT_GETFONTNAME
    case EW_GETFONTNAME:
      {
        TCHAR *version=var0;
        TCHAR* buf1=GetStringFromParm(0x11);
        if (!GetTTFFontName(buf1, version, NSIS_MAX_STRLEN))
        {
          exec_error++;
        }
        log_printf3(_T("GetTTFFontName(%s) returned %s"),buf1,version);
      }
      break;
#endif//NSIS_SUPPORT_GETFONTNAME
...
#ifdef _UNICODE
    // Jim Park: We are making FPUTS and FGETS work on ANSI files as before,
    // but in Unicode NSIS, the strings will be internally converted to
    // wchar_t* strings.  To work on Unicode files, I'm providing
    // FPUTWS and FGETWS.  Note that on the ANSI NSIS, the Unicode commands
    // will not exist.
    case EW_FPUTS:
      {
        DWORD dw;
        int l;
        char* ansiBuf;  // ANSI scratchpad
        TCHAR *t=var0;

        ansiBuf = (char*) GlobalAlloc(GPTR, NSIS_MAX_STRLEN);

        if (parm2) // WriteByte
        {
          ((unsigned char *)ansiBuf)[0]=GetIntFromParm(1)&0xff;
          l=1;
        }
        else
        {
          // The following line not only expands the variables in buf1 but
          // it also counts the number of characters.
          GetStringFromParm(0x11);
          WideCharToMultiByte(CP_ACP, 0, buf1, -1,
               ansiBuf, sizeof(ansiBuf[0])*NSIS_MAX_STRLEN,
               NULL, NULL);
          l=lstrlenA(ansiBuf);
        }
        if (!*t || !WriteFile((HANDLE)myatoi(t), ansiBuf, l, &dw, NULL))
        {
          exec_error++;
        }
        GlobalFree((HGLOBAL)ansiBuf);
      }
    break;
    case EW_FGETS:
      {
        ...
        if (*hptr)
        {
          ...
          while (rpos<maxlen)
          {
            ...
            MultiByteToWideChar(CP_ACP, 0, &c, sizeof(c), &wc, sizeof(wc));

            if (lc == '\r' || lc == '\n')
            {
              if (lc == c || (c != '\r' && c != '\n')) SetFilePointer(h,-((int)(sizeof(c))),NULL,FILE_CURRENT);
              else textout[rpos++]=wc;
              break;
            }
            textout[rpos++]=wc;
            lc=c;
    ...
    case EW_FPUTWS:
      {
        ...
        if (parm2)
        {
          ((_TUCHAR *)buf1)[0]=GetIntFromParm(1)&0xffff;
          l=1;
        }
        else
        {
