...
int StringList::find(const TCHAR *str, int case_sensitive, int *idx/*=NULL*/) const // returns -1 if not found
{
  ...
  while (offs < ml)
  {
    ...
    if (case_sensitive==2 &&
        str_slen < _tcsclen(s+offs) &&  // check for end of string
...
int StringList::idx2pos(int idx) const
{
  ...
  size_t offs=0;
  size_t cnt=0;
  if (idx>=0) while (offs < (m_gr.getlen()/sizeof(TCHAR)))
  {
    if (cnt++ == idx) return offs;
    offs+=_tcsclen(s+offs)+1;
  }
  return -1;
}
...
{
  TCHAR *s=(TCHAR*) m_gr.get();
  int ml=(m_gr.getlen()/sizeof(TCHAR));
  int offs=0;
  int idx=0;
  while (offs < ml)
  {
