...
#ifdef _UNICODE
  {_T("^Branding"), _T("Nullsoft Install System (Unicode) %s"), BOTH_STATIC},
#else
...
int StringsArray::set(int idx, const TCHAR *str)
{
  ...
  ((int*) m_offsets.get())[idx] = (m_strings.add(str, (_tcsclen(str) + 1)*sizeof(TCHAR))
                                    / sizeof(TCHAR));
...
{
  if ((unsigned int)idx >= (m_offsets.getlen() / sizeof(int)) || idx < 0)
    return 0;
...
/**
 * If we want a threadsafe version, we need the user to provide us the
 * buffer to write the Language Name to.
 */
const TCHAR *CEXEBuild::GetLangNameAndCP(LANGID lang, unsigned int *codepage/*=NULL*/) {
  static TCHAR szLangName[1024];  /* Not threadsafe because of this. */
...
#define ADD_FONT(id) { \
          ...
          if (dlg) { \
            ...
            DWORD dwSize; \
            dlg = td.Save(dwSize); \
            res_editor->UpdateResourceW(RT_DIALOG, MAKEINTRESOURCE(id+cur_offset), NSIS_DEFAULT_LANG, dlg, dwSize); \
            delete [] dlg; \
          } \
        }
#else
