...
#ifdef _UNICODE
  re->UpdateResourceW(RT_BITMAP, MAKEINTRESOURCEW(id), NSIS_DEFAULT_LANG, bitmap, dwSize);
#else
...
#ifdef _UNICODE
// Jim Park: This system call pipes in the STDOUT so that
// it can do a conversion to Unicode for proper display
// especially for makensisw.exe.
int MySystemCall(const TCHAR* command)
{
  ...
  // Jim Park: Since MySystemCall is calling CreateProcess with cmd.exe,
  // we need to provide the /C switch ourselves.
  TCHAR *cmdline = (TCHAR*) malloc( sizeof(TCHAR)*
                                    ( lstrlen(command) +
                                      lstrlen(_T("cmd.exe /C ")) +
                                      1 ) );
  lstrcpy(cmdline, _T("cmd.exe /C "));
  ...
  while (dwExit == STILL_ACTIVE || dwRead) 
  {
    ...
    if (dwRead)
    {
      ...
      wchar_t wBuf[sizeof(szBuf)];  // need double the size here.
      if (MultiByteToWideChar(CP_ACP, 0, (LPCSTR) szBuf, -1, wBuf, sizeof(wBuf)) != 0)
      {
        _fputts(wBuf, g_output);
        fflush(g_output);
      }
    }
    else Sleep(TIMEOUT);
...
#ifdef _UNICODE
  return MySystemCall(command_s.c_str());
#else
