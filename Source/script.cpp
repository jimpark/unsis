...
{
#ifdef _UNICODE
CValidateUnicode::FILE_TYPE ValidateUnicodeLicenseText(TCHAR*& buffer, unsigned int& datalen)
{
  unsigned char* ldata = (unsigned char*)(buffer + 1);
  CValidateUnicode::FILE_TYPE ftype = CValidateUnicode::CheckBOM(ldata, datalen);
  *buffer = SF_TEXT | SF_UNICODE;
      
  if (ftype == CValidateUnicode::UTF_8)
  {
    if (CValidateUnicode::ValidateUTF8(ldata, datalen))
    {
    ...
    {
      ftype = CValidateUnicode::UNKNOWN;
    }
  }
  else if (ftype == CValidateUnicode::UTF_16LE)
  {
  ...
  else if (ftype == CValidateUnicode::UTF_16BE)
  {
  ...
  else if (ftype == CValidateUnicode::UNKNOWN)
  {
    // No BOM so try to see if the text can be validated.
    if (CValidateUnicode::ValidateUTF8(ldata, datalen))
    {
      ftype = CValidateUnicode::UTF_8;
    }
    else if (CValidateUnicode::ValidateUTF16LE(ldata, datalen))
    {
  ...
  if (ftype == CValidateUnicode::UTF_8)
  {
    ...
    int wsize = ::MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)ldata, -1, NULL, 0);
    TCHAR* newBuf = (TCHAR*)malloc((wsize + 1) * sizeof(wchar_t));
    ::MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)ldata, -1, newBuf+1, wsize);
    *newBuf = SF_TEXT | SF_UNICODE;
    std::swap(newBuf, buffer);
    free(newBuf);
  }
...
parse_again:
  ...
  if (tkid == TOK_P_ELSE)
  {
    ...
    if (!num_ifblock())
    {
      ERROR_MSG(_T("!else: no if block _topen (!if[macro][n][def])\n"));
      return PS_ERROR;
    }
...
{
  std::vector<TCHAR> buffer(MAX_LINELENGTH);
  TCHAR* str = &buffer[0];
...
int CEXEBuild::process_jump(LineParser &line, int wt, int *offs)
{
  ...
  else
  {
    ...
    {
      ERROR_MSG(_T("Error: Goto targets beginning with _T('+') or _T('-') must be followed by nonzero integer (relative jump)\n"));
      return 1;
    }
...
int CEXEBuild::doCommand(int which_token, LineParser &line)
{
  ...
  switch (which_token)
  {
    ...
    case TOK_P_INSERTMACRO:
      {
        ...
        wsprintf(&str[0],_T("macro:%s"),line.gettoken_str(1));
        while (*t)
        {
          lp++;
          if (_tcscmp(t,_T(" ")))
          {
            int ret=process_oneline(t,&str[0],lp);
            if (ret != PS_OK)
            {
    ...
    case TOK_P_TEMPFILE:
      {
        ...
        std::vector<TCHAR> buf2(MAX_PATH);

        GetTempPath(MAX_PATH, &buf[0]);
        if (!GetTempFileName(&buf[0], _T("nst"), 0, &buf2[0]))
        {
    ...
    case TOK_LICENSELANGSTRING:
    {
      ...
      // Need to compare the following as chars, not TCHARs.
      if (!strncmp((char*)ldata,"{\\rtf",sizeof("{\\rtf")-1))
      {
#ifdef _UNICODE
      // We need to make the string WCHAR unfortunately.  This is
      // because the data structure we are using to store the string
      // can only really handle one kind of a string.  It's not a
      // raw storage but assumes things.

        WCHAR* tmp = winchar_fromansi((char*)ldata);
        free(data);
        data = (TCHAR*)malloc((winchar_strlen(tmp) + 2) * sizeof(TCHAR));
        *data = SF_RTF;
        winchar_strcpy(data+1, tmp);
        delete[] tmp;
#else
        *data = SF_RTF;
#endif
      ...
      else
      {
        *data = SF_TEXT;
#ifdef _UNICODE
        CValidateUnicode::FILE_TYPE ftype =
          ValidateUnicodeLicenseText(data, datalen);
        ...
        {
          SCRIPT_MSG(_T("LicenseLangString: %d Unicode encoding detected in \"%s\" as %s.\n"),
            lang, file, CValidateUnicode::TypeToName(ftype));
        }
#endif
      }
    ...
    /* Jim Park: License Data is tricky here.  We do want the license data
       to also support Unicode but we also want to support RTF.  Since
       RTF is strictly ASCII-based, we can't assume the file to be
       Unicode. */
       {
        int idx = 0;
        TCHAR *file = line.gettoken_str(1);
        ...
        if (!idx)
        {
          ...
          else
          {
            *data = SF_TEXT;
            ...
            {
              SCRIPT_MSG(_T("LicenseData: Unicode encoding detected in \"%s\" as %s.\n"), file,
                CValidateUnicode::TypeToName(ftype));
            }
#endif
          }
        }
    ...
    case TOK_INSTPROGRESSFLAGS:
      {
        ...
        try {
...
#ifdef _UNICODE
          BYTE* dlg = res_editor->GetResourceW(RT_DIALOG, MAKEINTRESOURCE(IDD_INSTFILES), NSIS_DEFAULT_LANG);
#else
          ...
          dlg = dt.Save(dwSize);
#ifdef _UNICODE
          res_editor->UpdateResourceW(RT_DIALOG, MAKEINTRESOURCE(IDD_INSTFILES), NSIS_DEFAULT_LANG, dlg, dwSize);
#else
    ...
    case TOK_CHANGEUI:
      try {
...
#ifdef _UNICODE
        #define GET(x) dlg = uire->GetResourceW(RT_DIALOG, MAKEINTRESOURCE(x), 0); if (!dlg) return PS_ERROR; CDialogTemplate UIDlg(dlg, uDefCodePage);
        ...
        #define SAVE(x) dwSize = UIDlg.GetSize(); res_editor->UpdateResourceW(RT_DIALOG, x, NSIS_DEFAULT_LANG, dlg, dwSize); delete [] dlg;
#else
    ...
    case TOK_ADDBRANDINGIMAGE:
      ...
      try {
        ...
        init_res_editor();
#ifdef _UNICODE
        BYTE* dlg = res_editor->GetResourceW(RT_DIALOG, MAKEINTRESOURCE(IDD_INST), NSIS_DEFAULT_LANG);
#else
...
#ifdef _UNICODE
        res_editor->UpdateResourceW(RT_DIALOG, IDD_INST, NSIS_DEFAULT_LANG, dlg, dwDlgSize);
#else
    ...
    case TOK_LOADNLF:
    {
      ...
      TCHAR lang_id[16];
      TCHAR lang_cp[16];
      std::vector<TCHAR> lang_name(1024);
      wsprintf(&lang_name[0], _T("LANG_%s"), table->nlf.m_szName);
      wsprintf(lang_id, _T("%u"), table->lang_id);
      wsprintf(lang_cp, _T("%u"), table->nlf.m_uCodePage);
      definedlist.add(&lang_name[0], lang_id);
      wsprintf(&lang_name[0], _T("LANG_%s_CP"), table->nlf.m_szName);
      definedlist.add(&lang_name[0], lang_cp);
    }
    return PS_OK;
    ...
    case TOK_P_DEFINE:
    {
      ...
      if (!_tcsicmp(define,_T("/date")) || !_tcsicmp(define,_T("/utcdate"))) {
        ...
        valuebuf[0]=0;
        size_t s=_tcsftime(&valuebuf[0],valuebuf.size(),value,localtime(&rawtime));

        if (s == 0)
          valuebuf[0]=0;
        else
          valuebuf[max(s,valuebuf.size()-1)]=0;
        ...
        if (fp)  {
          std::vector<TCHAR> str(MAX_LINELENGTH);
          for (;;) {
            TCHAR *p=&str[0];
            *p=0;
            _fgetts(&str[0],MAX_LINELENGTH,fp);
            linecnt++;
            if (feof(fp)&&!str[0]) break;

            while (*p) p++;
            if (p > &str[0]) p--;
            while (p >= &str[0] && (*p == _T('\r') || *p == _T('\n'))) p--;
            *++p=0;
            if (file_buf.getlen()) file_buf.add(_T("\n"),sizeof(TCHAR)); // only add the '\n'
            file_buf.add(&str[0],(_tcsclen(&str[0]))*sizeof(TCHAR));
          }
          fclose(fp);
        }
      ...
      } else if (_tcsicmp(define,_T("/file_version")) == 0 ||
                 _tcsicmp(define,_T("/product_version")) == 0) {
        if (line.getnumtokens() != 5) PRINTHELP()

        TCHAR* type = define;
        TCHAR* filename = line.gettoken_str(3);
        int    offset = line.gettoken_int(4);
        value = &valuebuf[0];
        define = line.gettoken_str(2);
          ...
          { 
            ...
            { 
              ...
              { 
                ...
                { 
                  if (_tcsicmp(type,_T("/file_version")) == 0)
                  {
                    int ver = 0;

                    if (offset == 0)
                    {
                      ver = HIWORD(verInfo->dwFileVersionMS); 
                    }
                    else if (offset == 1)
                    {
                      ver = LOWORD(verInfo->dwFileVersionMS); 
                    }
                    else if (offset == 2)
                    {
                      ver = HIWORD(verInfo->dwFileVersionLS); 
                    }
                    else if (offset == 3)
                    {
                      ver = LOWORD(verInfo->dwFileVersionLS);
                    }
                    else
                    {
                      PRINTHELP()
                    ...
                    _stprintf(value,_T("%d"),ver);
                  }
                  else if (_tcsicmp(type,_T("/product_version")) == 0)
                  {
                    int ver = 0;

                    if (offset == 0)
                    {
                      ver = HIWORD(verInfo->dwProductVersionMS); 
                    }
                    else if (offset == 1)
                    {
                      ver = LOWORD(verInfo->dwProductVersionMS); 
                    }
                    else if (offset == 2)
                    {
                      ver = HIWORD(verInfo->dwProductVersionLS); 
                    }
                    else if (offset == 3)
                    {
                      ver = LOWORD(verInfo->dwProductVersionLS);
                    }
                    else
                    {
                      PRINTHELP()
          ...
          for (;;)
          {
            ...
            for (;;)
            {
              ...
              *++p=0;
              ...
              bool endSlash = (str[0] && str[_tcsclen(&str[0])-1] == _T('\\'));
              if (tmpstr.getlen() || endSlash) tmpstr.add(&str[0],sizeof(TCHAR)*(_tcsclen(&str[0])));
            ...
            TCHAR *thisline=&str[0];
            if (tmpstr.getlen()) 
            {
              tmpstr.add(_T("\0"),sizeof(TCHAR)); // Just add the \0
    ...
    case TOK_P_SEARCHREPLACESTRING:
      {
        ...
        valout.add(_T("\0"),sizeof(TCHAR)); // Just add the \0
    ...
    case TOK_SUBSECTION:
    {
      ...
      wsprintf(&buf[0],_T("-%s"),line.gettoken_str(a));
      ...
      if (line.gettoken_str(a+1)[0]) SCRIPT_MSG(_T(" ->(%s)"),line.gettoken_str(a+1));
      SCRIPT_MSG(_T("\n"));
      return add_section(&buf[0],line.gettoken_str(a+1),ex);
    }
    case TOK_FUNCTION:
      if (!line.gettoken_str(1)[0]) PRINTHELP()
    ...
    case TOK_BRANDINGTEXT:
      {
        ...
        if (trim) try {
          ...
          if (trim) {
            std::vector<TCHAR> str(512);
            if (line.getnumtokens()==a+1 && line.gettoken_str(a)[0])
              _tcscpy(&str[0], line.gettoken_str(a));
            else
              wsprintf(&str[0], _T("Nullsoft Install System %s"), NSIS_VERSION);

            short old_width = td.GetItem(IDC_VERSTR)->sWidth;

            switch (trim) {
              case 1: td.LTrimToString(IDC_VERSTR, &str[0], 4); break;
              case 2: td.RTrimToString(IDC_VERSTR, &str[0], 4); break;
              case 3: td.CTrimToString(IDC_VERSTR, &str[0], 4); break;
            }

            if (td.GetItem(IDC_VERSTR)->sWidth > old_width)
            {
              warning_fl(_T("BrandingText: \"%s\" is too long, trimming has expanded the label"), &str[0]);
            }
          }
          ...
          dlg = td.Save(dwSize);
#ifdef _UNICODE
          res_editor->UpdateResourceW(RT_DIALOG, MAKEINTRESOURCE(IDD_INST), NSIS_DEFAULT_LANG, dlg, dwSize);
#else
    ...
    case TOK_CREATEDIR:
      {
        std::vector<TCHAR> out_path(1024);
        TCHAR *p=line.gettoken_str(1);
        if (*p == _T('-')) out_path[0]=0;
        else
        {
          if (p[0] == _T('\\') && p[1] != _T('\\')) p++;
          _tcsnccpy(&out_path[0],p,1024-1);
          if (*CharPrev(&out_path[0],&out_path[0]+_tcsclen(&out_path[0]))==_T('\\'))
            *CharPrev(&out_path[0],&out_path[0]+_tcsclen(&out_path[0]))=0; // remove trailing slash
        }
        if (!out_path[0]) PRINTHELP()
        SCRIPT_MSG(_T("CreateDirectory: \"%s\"\n"),&out_path[0]);
        ent.which=EW_CREATEDIR;
        ent.offsets[0]=add_string(&out_path[0]);
    ...
    case TOK_MESSAGEBOX:
#ifdef NSIS_SUPPORT_MESSAGEBOX
      {
        ...
        while (*p)
        {
          ...
          while (*np && *np != _T('|')) np++;
          if (*np) *np++=0;
          for (x = 0 ; (unsigned) x < sizeof(list) / sizeof(list[0]) && _tcsicmp(list[x].str, p); x++);
    ...
    return add_entry(&ent);
    case TOK_GETFONTVERSIONLOCAL:
      {
        TCHAR buf[129];
        if (GetTTFVersionString(line.gettoken_str(1), buf, 129))
        {
          ent.which=EW_ASSIGNVAR;
          ent.offsets[0]=GetUserVarIndex(line, 2);
          ent.offsets[1]=add_string(buf);
          ...
          ent.offsets[3]=0;
          if (ent.offsets[0]<0) PRINTHELP()
          SCRIPT_MSG(_T("GetFontVersionLocal: %s (%s)->(%s)\n"),
            line.gettoken_str(1),buf,line.gettoken_str(2));
          return add_entry(&ent);
        }
        else
        {
          ERROR_MSG(_T("Error: GetFontVersionLocal on font file '%s' failed.\n"), line.gettoken_str(1));
          return PS_ERROR;
        }
      }
    case TOK_GETFONTNAMELOCAL:
      {
        TCHAR buf[129];
        if (GetTTFFontName(line.gettoken_str(1), buf, 129))
        {
          ent.which=EW_ASSIGNVAR;
          ent.offsets[0]=GetUserVarIndex(line, 2);
          ent.offsets[1]=add_string(buf);
          ...
          ent.offsets[3]=0;
          if (ent.offsets[0]<0) PRINTHELP()
          SCRIPT_MSG(_T("GetFontNameLocal: %s (%s)->(%s)\n"),
            line.gettoken_str(1),buf,line.gettoken_str(2));
          return add_entry(&ent);
        }
        else
        {
          ERROR_MSG(_T("Error: GetFontNameLocal on font file '%s' failed.\n"), line.gettoken_str(1));
          return PS_ERROR;
        }
      }
    break;
    ...
    case TOK_GETFONTVERSION:
#ifdef NSIS_SUPPORT_GETFONTVERSION
      ent.which=EW_GETFONTVERSION;
      ent.offsets[0]=GetUserVarIndex(line, 2);
      ent.offsets[1]=add_string(line.gettoken_str(1));
      ent.offsets[2]=0;
      if (ent.offsets[0]<0) PRINTHELP()
      SCRIPT_MSG(_T("GetFontVersion: %s->%s\n"),
        line.gettoken_str(1), line.gettoken_str(2));
    return add_entry(&ent);
#else//!NSIS_SUPPORT_GETFONTVERSION
      ERROR_MSG(_T("Error: %s specified, NSIS_SUPPORT_GETFONTVERSION not defined.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif
    case TOK_GETFONTNAME:
#ifdef NSIS_SUPPORT_GETFONTNAME
      ent.which=EW_GETFONTNAME;
      ent.offsets[0]=GetUserVarIndex(line, 2);
      ent.offsets[1]=add_string(line.gettoken_str(1));
      ent.offsets[2]=0;
      if (ent.offsets[0]<0) PRINTHELP()
      SCRIPT_MSG(_T("GetFontName: %s->%s\n"),
        line.gettoken_str(1), line.gettoken_str(2));
    return add_entry(&ent);
#else//!NSIS_SUPPORT_GETFONTNAME
      ERROR_MSG(_T("Error: %s specified, NSIS_SUPPORT_GETFONTNAME not defined.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif
    ...
    case TOK_WRITEREGDWORD:
      {
        ...
        if (which_token == TOK_WRITEREGBIN)
        {
          ...
          std::vector<char> data(3*NSIS_MAX_STRLEN);
          ...
          SCRIPT_MSG(_T("WriteRegBin: %s\\%s\\%s=%s\n"),
            line.gettoken_str(1),line.gettoken_str(2),line.gettoken_str(3),line.gettoken_str(4));
          ent.offsets[3]=add_db_data(&data[0],data_len);
    ...
    case TOK_READENVSTR:
      ...
      ent.offsets[0]=GetUserVarIndex(line, 1);
      {
        std::vector<TCHAR> str(NSIS_MAX_STRLEN);
        _tcscpy(&str[0], _T("%"));
        _tcscat(&str[0], line.gettoken_str(2));
        _tcscat(&str[0], _T("%"));
        ent.offsets[1]=add_string(&str[0]);
    ...
    case TOK__PLUGINCOMMAND:
    {
      ...
      std::vector<TCHAR> tempDLL(NSIS_MAX_STRLEN);
      tstring dllName = get_file_name(dllPath);
      wsprintf(&tempDLL[0], _T("$PLUGINSDIR\\%s"), dllName.c_str());
      ...
      if (data_handle == -1)
      {
        ...
        ret=do_add_file(dllPath.c_str(),0,0,&files_added,&tempDLL[0],2,&data_handle); // 2 means no size add
        if (ret != PS_OK) {
          return ret;
        }
      ...
      else
      {
        ...
        ent.offsets[1]=add_string(&tempDLL[0]);
      ...
      // next, call it
      ent.which=EW_REGISTERDLL;
      ent.offsets[0]=add_string(&tempDLL[0]);;
    ...
    case TOK_FINDPROC:
      ent.which=EW_FINDPROC;
      ent.offsets[0]=GetUserVarIndex(line, 1); // output
      ent.offsets[1]=add_string(line.gettoken_str(2));
      SCRIPT_MSG(_T("FindProc output=%s process_name=%s\n"),line.gettoken_str(1), line.gettoken_str(2));
    return add_entry(&ent);
