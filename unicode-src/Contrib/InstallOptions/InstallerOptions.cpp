    ...
    if (pField->nType >= FIELD_CHECKLEN) {
      ...
      if (((pField->nMaxLength > 0) && (nLength > pField->nMaxLength)) ||
        ...
        if (pField->pszValidateText) {
          TCHAR szTitle[1024];
          GetWindowText(hMainWindow, szTitle, sizeof(szTitle)/sizeof(TCHAR));
...
int WINAPI ReadSettings(void) {
  ...
  for (nIdx = 0, nCtrlIdx = 0; nCtrlIdx < nNumFields; nCtrlIdx++, nIdx++) {
    ...
    {
      int nResult = GetPrivateProfileString(szField, _T("Filter"), _T("All Files|*.*"), szResult, sizeof(szResult)/sizeof(szResult[0]), pszFilename);
      if (nResult) {
...
LRESULT WINAPI WMCommandProc(HWND hWnd, UINT id, HWND hwndCtl, UINT codeNotify) {
  ...
  switch (pField->nType) {
    case FIELD_FILEREQUEST: {
      ...
      ofn.nMaxFile  = sizeof(szBrowsePath)/sizeof(TCHAR);
      ...
      GetWindowText(pField->hwnd, szBrowsePath, sizeof(szBrowsePath)/sizeof(TCHAR));
...
int mystrcmpi(TCHAR* a, TCHAR* b)
{
   int result = 0;

   while (*a != 0 && *b != 0 && result == 0)
   {
      TCHAR a_up = mytoupper(*a);
      TCHAR b_up = mytoupper(*b);
...
int WINAPI LookupToken(TableEntry* psTable_, TCHAR* pszToken_)
{
  for (int i = 0; psTable_[i].pszName; i++)
    if (!mystrcmpi(pszToken_, psTable_[i].pszName))
