...
{
  _ftprintf(g_output,_T("MakeNSIS %s - Copyright 1995-2015 Contributors\n")
...
{
  _ftprintf(g_output,_T("Copyright (C) 1999-2015 Nullsoft and Contributors\n\n")
...
int _tmain(int argc, TCHAR **argv)
{
  ...
  if (argc > 1 && IS_OPT(argv[1]) && !_tcsicmp(&argv[1][1],_T("RAW")))
  {
    // Set the output to RAW mode if we are using pipes etc.
    _setmode(_fileno(g_output), _O_BINARY);
    tmpargpos++;
    argpos++;
    raw_output = true;
  }
