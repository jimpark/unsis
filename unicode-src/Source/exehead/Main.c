...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPTSTR lpszCmdParam, int nCmdShow)
{
...

  {
    ...
    LoadSystemLibrary(_T("UXTHEME")); // On Vista OleInitialize calls NtUserCreateWindowEx and that pulls in UXTheme.dll
    LoadSystemLibrary(_T("USERENV")); // On Vista SHGetFileInfo ends up in SHELL32.kfapi::GetUserProfileDir and that pulls in UserEnv.dll
    LoadSystemLibrary(_T("SETUPAPI")); // On XP SHGetFileInfo ends up in CMountPoint::_InitLocalDriveHelper and that pulls in SetupAPI.dll
  }
