...
struct MGA_FUNC MGA_FUNCS[] = {
  {_T("KERNEL32"), "SetDefaultDllDirectories"},
  ...
  {_T("ADVAPI32"), "InitiateShutdownW"},
  {_T("SHELL32"), (CHAR*) 680}, // IsUserAnAdmin
  {_T("SHLWAPI"),  "SHAutoComplete"},
  {_T("SHFOLDER"), "SHGetFolderPathW"},
#ifdef NSIS_SUPPORT_GETDLLVERSION
  {_T("VERSION"),  "GetFileVersionInfoSizeW"},
  {_T("VERSION"),  "GetFileVersionInfoW"},
  {_T("VERSION"),  "VerQueryValueW"}
#endif
};
#else
struct MGA_FUNC MGA_FUNCS[] = {
  ...
  {_T("ADVAPI32"), "AdjustTokenPrivileges"},
  {_T("ADVAPI32"), "InitiateShutdownA"},
  {_T("SHELL32"), (CHAR*) 680}, // IsUserAnAdmin
  {_T("SHLWAPI"),  "SHAutoComplete"},
  {_T("SHFOLDER"), "SHGetFolderPathA"},
#ifdef NSIS_SUPPORT_GETDLLVERSION
  {_T("VERSION"),  "GetFileVersionInfoSizeA"},
  {_T("VERSION"),  "GetFileVersionInfoA"},
  {_T("VERSION"),  "VerQueryValueA"}
#endif
};
#endif

HMODULE NSISCALL LoadSystemLibrary(LPCTSTR name)
{
  // JP: rewrote what was in NSIS 2.50 since it was completely hacky looking.
  TCHAR path[MAX_PATH];
  UINT cSystem = GetSystemDirectory(path, MAX_PATH);
  UINT cName = (UINT)lstrlen(name);

  if ((cSystem + cName) > MAX_PATH) {
    // If MAX_PATH was not large enough and we don't support \\?\ paths
    // so we have to settle for just the name.
    return LoadLibrary(name);
  } else {
    wsprintf(path + cSystem, _T("\\%s"), name);
...
{
  const TCHAR *dllname = MGA_FUNCS[func].dll;
  HMODULE hModule;

  hModule = GetModuleHandle(dllname);    // Avoid LoadLibrary if possible because 
