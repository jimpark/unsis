...
int CEXEBuild::add_string(const TCHAR *string, int process/*=1*/, WORD codepage/*=CP_ACP*/)
{
  ...
  return cur_strlist->add(buf.c_str(),2);
}
...
int CEXEBuild::preprocess_string(tstring& out, const TCHAR *in, WORD codepage/*=CP_ACP*/)
{
  const TCHAR *p=in;
  while (*p)
  {
    ...
    if (np - p > 1) // multibyte TCHAR
    {
      ...
      while (l--)
      {
        _TUCHAR i = (_TUCHAR)*p++;
        if (NS_IS_CODE(i)) {
          out.push_back((TCHAR)NS_SKIP_CODE);
        }
        out.push_back((TCHAR)i);
      }
      continue;
    }
          ...
          if ( *p )
          {
            ...
            while (pUserVarName > p)
            {
              ...
              if (idxUserVar >= 0)
              {
                ...
                out.push_back((TCHAR) NS_VAR_CODE); // Named user variable;
                WORD w = FIX_ENDIAN_INT16(CODE_SHORT(idxUserVar));
                for (int i = 0; i < sizeof(WORD)/sizeof(TCHAR); ++i)
                {
                   out.push_back(*((TCHAR*)(&w) + i));
                }
          ...
          if (!bProceced && *p)
          {
            ...
            while (pShellConstName > p)
            {
              ...
              if (idxConst >= 0)
              {
                ...
                out.push_back((TCHAR)NS_SHELL_CODE); // Constant code identifier
#ifdef _UNICODE
                // Let's squish this into one SHORT even if using wchart_t.
                out.push_back((TCHAR)(FIX_ENDIAN_INT16( (CSIDL_Value_all << 8)
                                                | CSIDL_Value_current ) ));
#else
                out.push_back((TCHAR)CSIDL_Value_current);
                out.push_back((TCHAR)CSIDL_Value_all);
#endif
          ...
          if ( !bProceced && *p == _T('(') )
          {
            ...
            if (pos)
            {
              *pos = 0;
              ...
              if (idx < 0)
              {
                out.push_back((TCHAR)NS_LANG_CODE); // Next word is lang-string Identifier
...
#ifdef _UNICODE
          res_editor->UpdateResourceW(RT_VERSION, 1, lang_id, (BYTE*)VerInfoStream.get(), VerInfoStream.getlen());
#else
...
#define REMOVE_ICON(id) if (disable_window_icon) { \
    BYTE* dlg = res_editor->GetResourceW(RT_DIALOG, MAKEINTRESOURCE(id), NSIS_DEFAULT_LANG); \
    if (dlg) { \
      ...
      if (dt.RemoveItem(IDC_ULICON)) { \
        ...
        DWORD dwSize; \
        dlg = dt.Save(dwSize); \
        res_editor->UpdateResourceW(RT_DIALOG, MAKEINTRESOURCE(id), NSIS_DEFAULT_LANG, dlg, dwSize); \
        delete [] dlg; \
      } \
    } \
  }
...
#ifdef NSIS_CONFIG_LICENSEPAGE
    if (!license_normal) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_LICENSE, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_LICENSE);
    if (!license_fsrb) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_LICENSE_FSRB, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_LICENSE_FSRB);
    if (!license_fscb) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_LICENSE_FSCB, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_LICENSE_FSCB);
#endif // NSIS_CONFIG_LICENSEPAGE
#ifdef NSIS_CONFIG_COMPONENTPAGE
    if (!selcom) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_SELCOM, NSIS_DEFAULT_LANG, 0, 0);
      res_editor->UpdateResourceW(RT_BITMAP, IDB_BITMAP1, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_SELCOM);
#endif // NSIS_CONFIG_COMPONENTPAGE
    if (!dir) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_DIR, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_DIR);
#ifdef NSIS_CONFIG_UNINSTALL_SUPPORT
    if (!uninstconfirm) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_UNINST, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_UNINST);
#endif // NSIS_CONFIG_UNINSTALL_SUPPORT
    if (!instlog) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_INSTFILES, NSIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_INSTFILES);
    if (!main) {
      res_editor->UpdateResourceW(RT_DIALOG, IDD_INST, NSIS_DEFAULT_LANG, 0, 0);
      if (!build_compress_whole && !build_crcchk)
        res_editor->UpdateResourceW(RT_DIALOG, IDD_VERIFY, NSIS_DEFAULT_LANG, 0, 0);
    }
#else // ifdef _UNICODE
#define REMOVE_ICON(id) if (disable_window_icon) { \
...
#ifdef _UNICODE
    res_editor->UpdateResourceW(MAKEINTRESOURCE(24), MAKEINTRESOURCE(1), NSIS_DEFAULT_LANG, (LPBYTE) manifest.c_str(), manifest.length());
#else
...
int CEXEBuild::write_output(void)
{
  ...
  // If the total size of the file is smaller than the header, then the PE header
  // refers to virtual address space that is bigger than the total size of the
  // file and causes Windows to think that the EXE is invalid. So we need to pad
  // the file with zeros.

  if (ftell(fp) < (long int) m_exehead_original_size)
  {
     while(ftell(fp) < (long int) m_exehead_original_size)
     {
        fputc(0, fp);
     }

     INFO_MSG(_T("Total size after padding: %10d\n"), ftell(fp));
  }
...
int CEXEBuild::DeclaredUserVar(const TCHAR *szVarName)
{
  ...
  else
  {
    while (*pVarName)
    {
      ...
      {
        ERROR_MSG(_T("Error: invalid characters in variable name \"%s\", use only characters [a-z][A-Z][0-9] and _T('_')\n"), szVarName);
        return PS_ERROR;
      }
      pVarName++;
    }
