...
#ifdef _UNICODE
    icon.data = re->GetResourceW(RT_ICON, MAKEINTRESOURCE(rsrc_id), NSIS_DEFAULT_LANG);
#else
...
void set_icon(CResourceEditor* re, WORD wIconId, IconGroup icon1, IconGroup icon2)
{
...
#ifdef _UNICODE
  re->UpdateResourceW(RT_GROUP_ICON, MAKEINTRESOURCE(wIconId), NSIS_DEFAULT_LANG, group1, group_size);
#else
  ...
  unsigned i = 1;
#ifdef _UNICODE
  while (re->UpdateResourceW(RT_ICON, MAKEINTRESOURCE(i++), NSIS_DEFAULT_LANG, 0, 0));
#else
...
#ifdef _UNICODE
    re->UpdateResourceW(RT_ICON, MAKEINTRESOURCE(size_index + 1), NSIS_DEFAULT_LANG, data, size);
#else
...
int generate_unicons_offsets(LPBYTE exeHeader, size_t exeHeaderSize, LPBYTE uninstIconData, WORD wIconId) {
  try
  {
...
#ifdef _UNICODE
    offset = re.GetResourceOffsetW(RT_GROUP_ICON, MAKEINTRESOURCE(wIconId), NSIS_DEFAULT_LANG);
#else
    ...
    *(LPDWORD) seeker = FIX_ENDIAN_INT32(offset);
    ...
    {
#ifdef _UNICODE
      offset = re.GetResourceOffsetW(RT_ICON, MAKEINTRESOURCE(icon_index), NSIS_DEFAULT_LANG);
#else
...
#ifdef _UNICODE
      DWORD real_size = re.GetResourceSizeW(RT_ICON, MAKEINTRESOURCE(icon_index), NSIS_DEFAULT_LANG);
#else
