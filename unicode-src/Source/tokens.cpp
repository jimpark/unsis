...
typedef struct 
{
  ...
  int index; // index in the tokenList
} tokenType;
...
{TOK_GETFONTVERSION,_T("GetFontVersion"),2,0,_T("file $(user_var: version string output)"),TP_CODE},
{TOK_GETFONTVERSIONLOCAL,_T("GetFontVersionLocal"),2,0,_T("localfile $(user_var: version string output)"),TP_CODE},
{TOK_GETFONTNAME,_T("GetFontName"),2,0,_T("file $(user_var: version string output)"),TP_CODE},
{TOK_GETFONTNAMELOCAL,_T("GetFontNameLocal"),2,0,_T("localfile $(user_var: version string output)"),TP_CODE},
...
{TOK_P_DEFINE,_T("!define"),1,4,_T("([/date|/utcdate] symbol [value]) | (/file symbol filename) | (/file_version symbol filename (0|1|2|3)) | (/product_version symbol filename (0|1|2|3)) | (/math symbol val1 OP val2)\n    OP=(+ - * / % & | ^)"),TP_ALL},
...
// Added by Jim Park 14 aug 2007
{TOK_FINDPROC, _T("FindProc"),2,0, _T("$(user_var: output) process_name"), TP_CODE},
};

struct TokenPred : public std::binary_function<const TCHAR*, const TCHAR*, bool>
{
...
struct TokenHash : public std::unary_function<const TCHAR*, std::size_t>
{
  result_type operator()(argument_type arg) const
  {
    ...
    {
      // Based on boost.
      seed ^= toupper(*i) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
      ++i;
    }
...
const TokenMap& InitTokenMap()
{
  ...
  for (int i = 0; i < TOK__LAST; ++i)
  {
    tokenlist[i].index = i;
    tokenMap[tokenlist[i].name] = &(tokenlist[i]);
  }
...
{
   static const TokenMap& tokenMap = InitTokenMap();
   return tokenMap;
}
...
void CEXEBuild::print_help(TCHAR *commandname)
{
  if (!commandname)
  {
    for (int i = 0; i < TOK__LAST; ++i)
    {
      ERROR_MSG(_T("%s %s\n"), tokenlist[i].name,tokenlist[i].usage_str);
    }
  }
  else
  {
    static const TokenMap& tokenMap = GetTokenMap();
    TokenMap::const_iterator iter = tokenMap.find(commandname);
...
{
  static const TokenMap& tokenMap = GetTokenMap();
  TokenMap::const_iterator iter = tokenMap.find(s);

  return (iter != tokenMap.end());
}
...
int CEXEBuild::get_commandtoken(TCHAR *s, int *np, int *op, int *pos)
{
  ...
  if (iter != tokenMap.end())
  {
     *np = iter->second->num_parms;
     *op = iter->second->opt_parms;
     *pos = iter->second->index;
     result = iter->second->id;
  }
