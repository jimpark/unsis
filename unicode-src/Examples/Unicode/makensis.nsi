...
InstallDir $PROGRAMFILES\NSIS\Unicode
...
!define MEMENTO_REGISTRY_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\NSIS Unicode"
...
${MementoSection} "NSIS Core Files (required)" SecCore
  ...
  SectionIn 1 2 3 RO
  SetOutPath $INSTDIR
  RMDir /r $SMPROGRAMS\NSIS\Unicode
  ...
  no_nsiopen:
  WriteRegStr HKCR "NSIS.Script\shell\ucompile" "" "Compile Unicode NSIS Script"
  WriteRegStr HKCR "NSIS.Script\shell\ucompile\command" "" '"$INSTDIR\makensisw.exe" "%1"'
  WriteRegStr HKCR "NSIS.Script\shell\ucompile-compressor" "" "Compile Unicode NSIS Script (Choose Compressor)"
  WriteRegStr HKCR "NSIS.Script\shell\ucompile-compressor\command" "" '"$INSTDIR\makensisw.exe" /ChooseCompressor "%1"'
...
!endif
  ...
  SetOutPath $INSTDIR
!ifndef NO_STARTMENUSHORTCUTS
  CreateShortCut "$SMPROGRAMS\NSIS Unicode.lnk" "$INSTDIR\NSIS.exe"
!endif

  CreateShortCut "$DESKTOP\NSIS Unicode.lnk" "$INSTDIR\NSIS.exe"
...
Section -post
  ...
  WriteRegStr HKLM "Software\NSIS\Unicode" "" $INSTDIR
!ifdef VER_MAJOR & VER_MINOR & VER_REVISION & VER_BUILD
  WriteRegDword HKLM "Software\NSIS\Unicode" "VersionMajor" "${VER_MAJOR}"
  WriteRegDword HKLM "Software\NSIS\Unicode" "VersionMinor" "${VER_MINOR}"
  WriteRegDword HKLM "Software\NSIS\Unicode" "VersionRevision" "${VER_REVISION}"
  WriteRegDword HKLM "Software\NSIS\Unicode" "VersionBuild" "${VER_BUILD}"
!endif
  ...
  WriteRegStr HKLM "${MEMENTO_REGISTRY_KEY}" "DisplayName" "Nullsoft Install System (Unicode)"
...
Function PageReinstall
  ...
  ReadRegStr $R0 HKLM "Software\NSIS\Unicode" ""

  ${If} $R0 == ""
    Abort
  ${EndIf}

  ReadRegDWORD $R0 HKLM "Software\NSIS\Unicode" "VersionMajor"
  ReadRegDWORD $R1 HKLM "Software\NSIS\Unicode" "VersionMinor"
  ReadRegDWORD $R2 HKLM "Software\NSIS\Unicode" "VersionRevision"
  ReadRegDWORD $R3 HKLM "Software\NSIS\Unicode" "VersionBuild"
...
Section Uninstall
  ...
  DeleteRegKey HKLM "Software\NSIS\Unicode"

  SetDetailsPrint textonly
  DetailPrint "Deleting Files..."
  SetDetailsPrint listonly

  Delete "$SMPROGRAMS\NSIS Unicode.lnk"
  Delete "$DESKTOP\NSIS Unicode.lnk"
