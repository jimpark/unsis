...
InstallDir $PROGRAMFILES\NSIS\ANSI
InstallDirRegKey HKLM Software\NSIS\ANSI ""
...
!define MEMENTO_REGISTRY_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\NSIS ANSI"
...
${MementoSection} "NSIS Core Files (required)" SecCore
  ...
  no_nsiopen:
  WriteRegStr HKCR "NSIS.Script\shell\compile" "" "Compile ANSI NSIS Script"
  WriteRegStr HKCR "NSIS.Script\shell\compile\command" "" '"$INSTDIR\makensisw.exe" "%1"'
  WriteRegStr HKCR "NSIS.Script\shell\compile-compressor" "" "Compile ANSI NSIS Script (Choose Compressor)"
...
!endif
  ...
  SetOutPath $INSTDIR
!ifndef NO_STARTMENUSHORTCUTS
  CreateShortCut "$SMPROGRAMS\NSIS ANSI.lnk" "$INSTDIR\NSIS.exe"
!endif

  CreateShortCut "$DESKTOP\NSIS ANSI.lnk" "$INSTDIR\NSIS.exe"
...
Section -post
  ...
  WriteRegStr HKLM "Software\NSIS\ANSI" "" $INSTDIR
!ifdef VER_MAJOR & VER_MINOR & VER_REVISION & VER_BUILD
  WriteRegDword HKLM "Software\NSIS\ANSI" "VersionMajor" "${VER_MAJOR}"
  WriteRegDword HKLM "Software\NSIS\ANSI" "VersionMinor" "${VER_MINOR}"
  WriteRegDword HKLM "Software\NSIS\ANSI" "VersionRevision" "${VER_REVISION}"
  WriteRegDword HKLM "Software\NSIS\ANSI" "VersionBuild" "${VER_BUILD}"
!endif
  ...
  WriteRegStr HKLM "${MEMENTO_REGISTRY_KEY}" "DisplayName" "Nullsoft Install System (ANSI)"
...
Function PageReinstall
  ...
  ReadRegStr $R0 HKLM "Software\NSIS\ANSI" ""

  ${If} $R0 == ""
    Abort
  ${EndIf}

  ReadRegDWORD $R0 HKLM "Software\NSIS\ANSI" "VersionMajor"
  ReadRegDWORD $R1 HKLM "Software\NSIS\ANSI" "VersionMinor"
  ReadRegDWORD $R2 HKLM "Software\NSIS\ANSI" "VersionRevision"
  ReadRegDWORD $R3 HKLM "Software\NSIS\ANSI" "VersionBuild"
...
Section Uninstall
  ...
  DeleteRegKey HKLM "Software\NSIS\ANSI"

  SetDetailsPrint textonly
  DetailPrint "Deleting Files..."
  SetDetailsPrint listonly

  Delete "$SMPROGRAMS\NSIS ANSI.lnk"
  Delete "$DESKTOP\NSIS ANSI.lnk"
