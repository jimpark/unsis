...
#ifdef _UNICODE
  // Moved up to RichEd20.dll from RichEd32.dll which is Rich Edit 1.0
  HINSTANCE hRichEditDLL = LoadLibrary(_T("RichEd20.dll"));
#else
  // Stay with the old Rich Edit 1.0.
...
#ifdef _UNICODE
      // Let's be nice and check to see if we are dealing with ANSI NSIS.
      if (!g_sdata.isNSISUnicode)
      {
        szBuf[dwRead] = 0;
        wchar_t wBuf[sizeof(szBuf)];  // need double the size here.
        if (MultiByteToWideChar(CP_ACP, 0, (LPCSTR) szBuf, -1, wBuf, sizeof(wBuf)) != 0)
        {
          LogMessage(g_sdata.hwnd, wBuf);
        }
      }
      else
      {
        wchar_t* ptr = (wchar_t*)szBuf;
        ptr[dwRead/sizeof(TCHAR)] = 0;
        LogMessage(g_sdata.hwnd, ptr);
      }
#else
      szBuf[dwRead] = 0;
