...
int SetArgv(const TCHAR *cmdLine, int *argc, TCHAR ***argv)
{
  ...
  argSpaceSize = (size+1) * sizeof(TCHAR *) + (lstrlen(cmdLine) + size) * sizeof(TCHAR);
  argSpace = (TCHAR *) GlobalAlloc(GMEM_FIXED, argSpaceSize);
  if (!argSpace)
    return 0;

  *argv = (TCHAR **) argSpace;
  argSpace = (TCHAR *)(((TCHAR **)argSpace) + size);
  size--;
...
void SetTitle(HWND hwnd,TCHAR *substr) {
  TCHAR title[64];
#ifdef _UNICODE
  if (substr==NULL) wsprintf(title,_T("MakeNSISW (Unicode)"));
  else wsprintf(title,_T("MakeNSISW (Unicode) l- %s"),substr);
#else
...
void CompileNSISScript() {
  ...
  if (!g_sdata.compile_command) {
    ...
    size_t byteSize = sizeof(TCHAR)*(
      ...
      /* /NOTTIFYHWND + HWND */ lstrlen(_T("/NOTIFYHWND -4294967295")) + /* space */ 1
      +6); /* for -- \"\" and NULL */
    ...
    size_t temp = (lstrlen(g_sdata.compile_command) + 1)*sizeof(TCHAR);
    if (byteSize < temp)
    {
      ErrorMessage(g_sdata.hwnd, _T("Bad memory allocation!"));
    }
...
void SaveWindowPos(HWND hwnd) {
  ...
  if (OpenRegSettingsKey(hKey, true)) {
    RegSetValueEx(hKey,REGLOC,0,REG_BINARY,(const BYTE*)(&p),sizeof(p));
    RegCloseKey(hKey);
  }
}
...
int InitBranding() {
  ...
  lstrcat(s, opt);
  {
    ...
    if (szBuf[0] >> 8)
    {
      ...
      wchar_t wBuf[1024];
      if (MultiByteToWideChar(CP_ACP, 0, (LPCSTR) szBuf, -1, wBuf, sizeof(wBuf)) != 0)
      {
        lstrcpy(szBuf, wBuf);
      }
    }
    ...
    {
      g_sdata.isNSISUnicode = true; // It's Unicode!
    }
#else
...
void PushMRUFile(TCHAR* fname)
{
  int i;
  DWORD   rv;
  //TCHAR file_part[MAX_PATH+1];   // Should be a buffer, not a pointer! -- Jim Park
  ...
  rv = GetFullPathName(fname,sizeof(full_file_name)/sizeof(TCHAR),full_file_name,NULL);
  if (rv == 0) {
    return;
  }
