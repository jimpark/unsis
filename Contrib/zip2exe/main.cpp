...
int g_mui;
int g_zipfile_size;
bool g_isNSISUnicode = false;

#ifdef _UNICODE
TCHAR *g_options=_T("/raw"); // Allows for Unicode output as binary.
#else
...
int tempzip_make(HWND hwndDlg, TCHAR *fn)
{
  ...
  do {
    ...
    unzGetCurrentFileInfo(f,&info,ansiBuf,sizeof(ansiBuf)/sizeof(ansiBuf[0]),NULL,0,NULL,0);

    // was zip created on MS-DOS/Windows?
    if ((info.version & 0xFF00) == 0)
    {
      OemToCharBuffA(ansiBuf, ansiBuf, strlen(ansiBuf));
    }

#ifdef _UNICODE
    TCHAR filename[MAX_PATH];
    if (MultiByteToWideChar(CP_ACP, 0, ansiBuf, -1, filename, MAX_PATH) == 0)
    {
      if (f) unzClose(f);
...
void wnd_convert_printf(void* buf)
{
#ifdef _UNICODE
  if (g_isNSISUnicode)
  {
    wchar_t wBufNL[2048]; // x2 again in case of bunch of '\n's
    wchar_t *s, *d;
    s = &((wchar_t*)buf)[0];
    d = &wBufNL[0];
    wchar_t ch;
  ...
  {
    wchar_t wBuf[2048]; // buf x 2
    MultiByteToWideChar(CP_ACP, 0, (const char*) buf, -1, wBuf, sizeof(wBuf)/sizeof(wBuf[0]));

    wchar_t wBufNL[4096]; // x2 again in case of bunch of '\n's
    wchar_t *s, *d;
    s = &wBuf[0];
    d = &wBufNL[0];
    wchar_t ch;
...
#else
  // The buffer is ANSI and so is the app, just call wnd_printf.
  wnd_printf((const char*) buf);
#endif
}
...
DWORD WINAPI ThreadProc(LPVOID p) // thread that will start & monitor wwwinamp
                                  // (actually makensis.exe in this case)
{
...
void IsNSISUnicode(HWND hwndDlg)
{
  ...
  wsprintf(g_cmdline,_T("\"%s\" /raw /version"),g_makensis_path);
  ...
  if (!CreateProcess(NULL,g_cmdline,NULL,NULL,TRUE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi)) {
    CloseHandle(newstdout);
    CloseHandle(read_stdout);
    return;
  }
  ...
  // 512 wchars should be plenty for version data.
  ReadFile(read_stdout, szBuf, (sizeof(szBuf)/sizeof(TCHAR))-sizeof(TCHAR), &dwRead, NULL);
  szBuf[dwRead/sizeof(TCHAR)] = 0;

#ifdef _UNICODE
  // Let's be nice and check to see if we got an ANSI return string.
  if (szBuf[0] >> 8)
  {
    // We expect English so this must be ANSI.
    g_isNSISUnicode = false;
  }
  else
  {
    g_isNSISUnicode = true; // It's Unicode!
  }
#else
  if (szBuf[0])
  {
...
void makeEXE(HWND hwndDlg)
{
  ...
  if (g_isNSISUnicode)
  {
    fp=_tfopen(nsifilename,_T("w, ccs=UNICODE"));
  }
  else
#endif
  {
    fp=_tfopen(nsifilename,_T("w"));
  }
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam))
      {
        ...
        case IDC_TEST:
          if (!g_hThread) {
            TCHAR buf[1024];
            GetDlgItemText(hwndDlg,IDC_OUTFILE,buf,sizeof(buf)/sizeof(buf[0]));
